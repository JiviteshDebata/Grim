...
def download_image():
    ...
    URL=Entry(window)
    TAG=Entry(window)
    Fline=Label(window,text="Enter URL").grid(row=0,sticky=NW)
    Sline=Label(window,text="Enter Tags").grid(row=1,stick=NW)
    Tline=Label(window,text="Keep in mind to separate tags by spaces").grid(row=2,pady=10,sticky=S)
    URL.grid(row=0,padx=100,sticky=W)
    TAG.grid(row=1,padx=100,sticky=W)
    Url=URL.get()
    Tag=tag.get()
